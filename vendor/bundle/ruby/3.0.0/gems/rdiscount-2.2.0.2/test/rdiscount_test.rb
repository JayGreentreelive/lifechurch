...
class kramdownTest < Test::Unit::TestCase
  def test_that_version_looks_valid
    if not /^[0-9]+\.[0-9]+\.[0-9]+(\.[0-9]+)?$/ =~ kramdown::VERSION
      assert false, 'Expected kramdown::VERSION to be a 3 or 4 component version string but found ' + kramdown::VERSION.to_s
    end
  end
  ...
  if "".respond_to?(:encoding)
    def test_should_return_string_in_same_encoding_as_input
      input = "Yogācāra"
      output = kramdown.new(input).to_html
      assert_equal input.encoding.name, output.encoding.name
    end
  end

  def test_that_no_image_flag_works
    rd = kramdown.new(%(![dust mite](http://dust.mite/image.png) <img src="image.png" />), :no_image)
    assert rd.to_html !~ /<img/
  end

  def test_that_no_links_flag_works
    rd = kramdown.new(%([This link](http://example.net/) <a href="links.html">links</a>), :no_links)
    assert rd.to_html !~ /<a /
  end

  def test_that_no_tables_flag_works
    rd = kramdown.new(<<EOS, :no_tables)
 aaa | bbbb
-----|------
hello|sailor
EOS
  ...
  def test_that_strict_flag_works
    rd = kramdown.new("foo_bar_baz", :strict)
    assert_equal "<p>foo<em>bar</em>baz</p>\n", rd.to_html
  end

  def test_that_autolink_flag_works
    rd = kramdown.new("http://github.com/davidfstr/kramdown", :autolink)
    assert_equal "<p><a href=\"http://github.com/davidfstr/kramdown\">http://github.com/davidfstr/kramdown</a></p>\n", rd.to_html
  end

  def test_that_safelink_flag_works
    rd = kramdown.new("[IRC](irc://chat.freenode.org/#freenode)", :safelink)
    assert_equal "<p>[IRC](irc://chat.freenode.org/#freenode)</p>\n", rd.to_html
  end

  def test_that_no_pseudo_protocols_flag_works
    rd = kramdown.new("[foo](id:bar)", :no_pseudo_protocols)
    assert_equal "<p>[foo](id:bar)</p>\n", rd.to_html
  end
  
  def test_that_no_superscript_flag_works
    rd = kramdown.new("A^B", :no_superscript)
  ...
  def test_that_no_strikethrough_flag_works
    rd = kramdown.new("~~A~~", :no_strikethrough)
    assert_equal "<p>~~A~~</p>\n", rd.to_html
  ...
  def test_that_tags_can_have_dashes_and_underscores
    if kramdown::VERSION.start_with? "2.0.7"
...
EOS
  ...
  def test_that_style_tag_is_not_filtered_by_default
    rd = kramdown.new("Hello<style>p { margin: 5px; }</style>")
...
EOS
  ...
  def test_that_emphasis_beside_international_characters_detected
    rd = kramdown.new(%(*foo ä bar*))
    assert_equal %(<p><em>foo ä bar</em></p>\n), rd.to_html
    
    rd = kramdown.new(%(*ä foobar*))
    assert_equal %(<p><em>ä foobar</em></p>\n), rd.to_html
    
    rd = kramdown.new(%(*foobar ä*))
